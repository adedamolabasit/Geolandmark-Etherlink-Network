...
export const BaseMap = () => {
  ...
  // });
  const convertedCoordinates = fetchedData.map((data) => {
    return data.geographicCoordinates.map((cord) => {
      const point = Object.values(cord)[0];
...
export const SingleBaseMap = ({ singleSelection }) => {
  const zoom = 24;
  const icon = L.icon({
    iconUrl: Logo,
    iconSize: [26, 26],
  ...
  return (
    <div className="w-[40.0521vw] h-[45.6481vh] bg-gray-100  rounded-[2vh]">
      <MapContainer
        center={location}
        zoom={zoom}
        className="h-[45.6481vh] rounded-[40px]"
      >
        <TileLayer
          url="https://tile.openstreetmap.org/${z}/${x}/${y}.png"
          attribution="© OpenStreetMap contributors"
        ...
        <Marker icon={icon} position={epcotCenter}>
          <Popup>
            {singleSelection?.fullName},{singleSelection?.address}
          </Popup>
        </Marker>
      </MapContainer>
    </div>
